...
class Email {
	...
	public function setOptions($userOptions){
		...
		//self::$options = array_replace(self::$options, $userOptions);
	...
	public function sendMail($data){
		...
		$headers .= ( $options['sendFromSite'] )
			? "From: =?utf-8?B?".base64_encode($options['siteName'])."?= <".$options['siteEmail'].">\r\n"
			: "From: =?utf-8?B?".base64_encode($data["userName"]["value"])."?= <".$data["userEmail"]["value"].">\r\n";
		// Добавление заголовка "Кому ответить"
		$headers .= ( $options['replyToSite'] )
			? "Reply-To: =?utf-8?B?".base64_encode($options['siteName'])."?= <".$options['siteEmail'].">\r\n"
		 ...
		 */
			...
			if ( mail($options['adminEmails'], $subject, $message, $headers) ) {
...
<html lang="ru">
    ...
    <body>
		...
		if ( !empty($_POST) && isset($_POST['sendMail']) ) {
			...
			//$sendMail->sendMail($data, 'Письмо с сайта "GL"');
